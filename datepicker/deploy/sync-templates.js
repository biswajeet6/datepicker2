...
const s3url = 's3://ENTER-DOMAIN-HERE'
...
const command = args[0] === 'local' ? `aws s3 sync ${s3url}/${process.env.STAGE} .serverless --delete` : `aws s3 sync .serverless ${s3url}/${process.env.STAGE} --delete`
